...
from kiwi.cmds.units import units
...
from kiwi.util.misc import format_bytes
from kiwi.util.misc import format_minutes
from kiwi.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total kiwi farmed: {amounts['farmed_amount'] / units['kiwi']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['kiwi']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['kiwi']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'kiwi start wallet' and 'kiwi wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'kiwi wallet show'")
    else:
        print("Note: log into your key using 'kiwi wallet show' to see rewards for each key")
