...
if [ "$NOTARIZE" ]; then
  electron-osx-sign Kiwi-darwin-arm64/Kiwi.app --platform=darwin \
  --hardened-runtime=true --provisioning-profile=kiwiblockchain.provisionprofile \
...
mv Kiwi-darwin-arm64 ../build_scripts/dist/
cd ../build_scripts || exit

DMG_NAME="Kiwi-$KIWI_INSTALLER_VERSION-arm64.dmg"
echo "Create $DMG_NAME"
mkdir final_installer
electron-installer-dmg dist/Kiwi-darwin-arm64/Kiwi.app Kiwi-$KIWI_INSTALLER_VERSION-arm64 \
...
if [ "$NOTARIZE" ]; then
  ...
  notarize-cli --file=$DMG_NAME --bundle-id net.kiwi.blockchain \
...
# xcrun altool --notarize-app -f Kiwi-0.1.X.dmg --primary-bundle-id net.kiwi.blockchain -u username -p password
