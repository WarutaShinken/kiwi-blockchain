...
# the output will be printed to your kiwi root path, e.g. ~/.kiwi/mainnet/profile/
# to analyze the profile, run:

#   python kiwi/utils/profiler.py ~/.kiwi/mainnet/profile | less -r

# this will print CPU usage of the kiwi full node main thread at 1 second increments.
# find a time window of interest and analyze the profile file (which are in pstats format).

# for example:

#   python kiwi/utils/profiler.py ~/.kiwi/mainnet/profile 10 20
...
if __name__ == "__main__":
    ...
    def analyze_slot_range(profile_dir: pathlib.Path, first: int, last: int):
        ...
        output_file = "kiwi-hotspot-%d" % first
        if first < last:
            output_file += "-%d" % last
