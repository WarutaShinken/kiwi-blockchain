...
from kiwi.cmds.init_funcs import create_all_ssl, create_default_kiwi_config
from kiwi.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from kiwi.util.errors import Err
from kiwi.full_node.generator import setup_generator_args
from kiwi.full_node.mempool_check_conditions import GENERATOR_MOD
...
from kiwi.consensus.blockchain_interface import BlockchainInterface
from kiwi.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
from kiwi.consensus.constants import ConsensusConstants
from kiwi.consensus.default_constants import DEFAULT_CONSTANTS
from kiwi.consensus.deficit import calculate_deficit
from kiwi.consensus.full_block_to_block_record import block_to_block_record
from kiwi.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from kiwi.consensus.cost_calculator import NPCResult, calculate_cost_of_program
from kiwi.consensus.pot_iterations import (
    calculate_ip_iters,
...
from kiwi.consensus.vdf_info_computation import get_signage_point_vdf_info
from kiwi.full_node.signage_point import SignagePoint
from kiwi.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
from kiwi.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
...
from kiwi.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
from kiwi.types.blockchain_format.sized_bytes import bytes32
from kiwi.types.blockchain_format.slots import (
...
from kiwi.types.blockchain_format.vdf import VDFInfo, VDFProof
...
from kiwi.types.generator_types import BlockGenerator, CompressorArg
...
from kiwi.util.block_cache import BlockCache
from kiwi.util.condition_tools import ConditionOpcode, conditions_by_opcode
from kiwi.util.config import load_config, save_config
from kiwi.util.hash import std_hash
from kiwi.util.ints import uint8, uint16, uint32, uint64, uint128
from kiwi.util.keychain import Keychain, bytes_to_mnemonic
from kiwi.util.merkle_set import MerkleSet
from kiwi.util.prev_transaction_block import get_prev_transaction_block
from kiwi.util.path import mkdir
from kiwi.util.vdf_prover import get_vdf_info_and_proof
from tests.wallet_tools import WalletTool
from kiwi.wallet.derive_keys import (
...
class BlockTools:
    ...
    def __init__(
        ...
        create_default_kiwi_config(root_path)
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `kiwi generate keys`")
    ...
    def init_plots(self, root_path: Path):
        ...
        try:
            ...
            args.pool_contract_address = encode_puzzle_hash(self.pool_ph, "xkw")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("KIWI_ROOT", "~/.kiwi/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
