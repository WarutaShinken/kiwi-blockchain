...
from kiwi.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from kiwi.protocols import wallet_protocol
from kiwi.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from kiwi.protocols.protocol_message_types import ProtocolMessageTypes
from kiwi.protocols.wallet_protocol import (
...
from kiwi.util.byte_types import hexstr_to_bytes
from kiwi.util.errors import Err, ValidationError
from kiwi.util.ints import uint32, uint128
from kiwi.util.keychain import Keychain
from kiwi.util.lru_cache import LRUCache
from kiwi.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from kiwi.util.path import mkdir, path_from_root
from kiwi.wallet.block_record import HeaderBlockRecord
from kiwi.wallet.derivation_record import DerivationRecord
from kiwi.wallet.settings.settings_objects import BackupInitialized
from kiwi.wallet.transaction_record import TransactionRecord
from kiwi.wallet.util.backup_utils import open_backup_file
from kiwi.wallet.util.wallet_types import WalletType
from kiwi.wallet.wallet_action import WalletAction
from kiwi.wallet.wallet_blockchain import ReceiveBlockResult
from kiwi.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'kiwi keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSKiwiConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSKiwiConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSKiwiConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSKiwiConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSKiwiConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSKiwiConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
