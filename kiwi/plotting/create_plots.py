...
from kiwi.plotting.plot_tools import add_plot_directory, stream_plot_info_ph, stream_plot_info_pk
...
from kiwi.util.config import config_path_for_filename, load_config
from kiwi.util.keychain import Keychain
from kiwi.util.path import mkdir
from kiwi.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk, master_sk_to_pool_sk
...
def get_farmer_public_key(alt_fingerprint: Optional[int] = None) -> G1Element:
    ...
    if sk_ent is None:
        raise RuntimeError("No keys, please run 'kiwi keys add', 'kiwi keys generate' or provide a public key with -f")
