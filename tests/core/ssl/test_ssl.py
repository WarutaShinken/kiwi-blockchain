...
from kiwi.protocols.shared_protocol import protocol_version
from kiwi.server.outbound_message import NodeType
from kiwi.server.server import KiwiServer, ssl_context_for_client
from kiwi.server.ws_connection import WSKiwiConnection
from kiwi.ssl.create_ssl import generate_ca_signed_cert
from kiwi.types.peer_info import PeerInfo
from tests.block_tools import test_constants
from kiwi.util.ints import uint16
...
async def establish_connection(server: KiwiServer, dummy_port: int, ssl_context) -> bool:
...
class TestSSL:
    ...
    async def test_public_connections(self, wallet_node):
        ...
        server_1: KiwiServer = full_node_api.full_node.server
    ...
    async def test_farmer(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            farmer_server.kiwi_ca_crt_path.read_bytes(), farmer_server.kiwi_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
            farmer_server.kiwi_ca_crt_path, farmer_server.kiwi_ca_crt_path, pub_crt, pub_key
        )
        connected = await establish_connection(farmer_server, 12312, ssl_context)
    ...
    async def test_full_node(self, wallet_node):
        ...
        generate_ca_signed_cert(
            full_node_server.kiwi_ca_crt_path.read_bytes(),
            full_node_server.kiwi_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            full_node_server.kiwi_ca_crt_path, full_node_server.kiwi_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_wallet(self, wallet_node):
        ...
        generate_ca_signed_cert(
            wallet_server.kiwi_ca_crt_path.read_bytes(), wallet_server.kiwi_ca_key_path.read_bytes(), pub_crt, pub_key
        )
        ssl_context = ssl_context_for_client(
            wallet_server.kiwi_ca_crt_path, wallet_server.kiwi_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_harvester(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            harvester_server.kiwi_ca_crt_path.read_bytes(),
            harvester_server.kiwi_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            harvester_server.kiwi_ca_crt_path, harvester_server.kiwi_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_introducer(self, introducer):
        ...
        # Create not authenticated cert
        pub_crt = introducer_server.kiwi_ca_key_path.parent / "p2p.crt"
        pub_key = introducer_server.kiwi_ca_key_path.parent / "p2p.key"
        generate_ca_signed_cert(
            introducer_server.kiwi_ca_crt_path.read_bytes(),
            introducer_server.kiwi_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            introducer_server.kiwi_ca_crt_path, introducer_server.kiwi_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_timelord(self, timelord):
        ...
        generate_ca_signed_cert(
            timelord_server.kiwi_ca_crt_path.read_bytes(),
            timelord_server.kiwi_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            timelord_server.kiwi_ca_crt_path, timelord_server.kiwi_ca_crt_path, pub_crt, pub_key
        )
