...
from kiwi.protocols import full_node_protocol, introducer_protocol
from kiwi.protocols.protocol_message_types import ProtocolMessageTypes
from kiwi.server.address_manager import AddressManager, ExtendedPeerInfo
from kiwi.server.address_manager_store import AddressManagerStore
...
class FullNodeDiscovery:
    ...
    async def on_connect(self, peer: ws.WSKiwiConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSKiwiConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSKiwiConnection):
