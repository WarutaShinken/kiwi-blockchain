...
from kiwi.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from kiwi.full_node.full_node_api import FullNodeAPI
from kiwi.server.start_farmer import service_kwargs_for_farmer
from kiwi.server.start_full_node import service_kwargs_for_full_node
from kiwi.server.start_harvester import service_kwargs_for_harvester
from kiwi.server.start_introducer import service_kwargs_for_introducer
from kiwi.server.start_service import Service
from kiwi.server.start_timelord import service_kwargs_for_timelord
from kiwi.server.start_wallet import service_kwargs_for_wallet
from kiwi.simulator.start_simulator import service_kwargs_for_full_node_simulator
from kiwi.timelord.timelord_launcher import kill_processes, spawn_process
...
async def setup_farmer(
    ...
    config["xkw_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xkw")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xkw_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xkw")
