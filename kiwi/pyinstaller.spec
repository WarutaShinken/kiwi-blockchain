...
ROOT = pathlib.Path(importlib.import_module("kiwi").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `kiwi` file name (which is the executable)
    and the `kiwi` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `kiwi` directory, since the names collide.
...
# TODO: collapse all these entry points into one `kiwi_exec` entrypoint that accepts the server as a parameter

entry_points = ["kiwi.cmds.kiwi"] + [f"kiwi.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "kiwi.util.bip39"])

if THIS_IS_WINDOWS:
    kiwi_mod = importlib.import_module("kiwi")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/kiwi/util/english.txt", "kiwi/util"))
datas.append((f"{ROOT}/kiwi/util/initial-config.yaml", "kiwi/util"))
datas.append((f"{ROOT}/kiwi/wallet/puzzles/*.hex", "kiwi/wallet/puzzles"))
datas.append((f"{ROOT}/kiwi/ssl/*", "kiwi/ssl"))
...
add_binary("kiwi", f"{ROOT}/kiwi/cmds/kiwi.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/kiwi/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/kiwi/server/start_{server}.py", COLLECT_ARGS)
