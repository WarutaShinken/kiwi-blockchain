...
from kiwi.cmds.init_funcs import kiwi_init
from kiwi.daemon.windows_signal import kill
from kiwi.server.server import ssl_context_for_root, ssl_context_for_server
from kiwi.ssl.create_ssl import get_mozilla_ca_crt
from kiwi.util.kiwi_logging import initialize_logging
from kiwi.util.config import load_config
from kiwi.util.json_util import dict_to_json_str
from kiwi.util.path import mkdir
from kiwi.util.service_groups import validate_service
from kiwi.util.setproctitle import setproctitle
from kiwi.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Kiwi.")
    quit()
...
service_plotter = "kiwi plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "kiwi": "kiwi",
        "kiwi_wallet": "start_wallet",
        "kiwi_full_node": "start_full_node",
        "kiwi_harvester": "start_harvester",
        "kiwi_farmer": "start_farmer",
        "kiwi_introducer": "start_introducer",
        "kiwi_timelord": "start_timelord",
        "kiwi_timelord_launcher": "timelord_launcher",
        "kiwi_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("kiwi daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered KIWI_ROOT
    os.environ["KIWI_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with KIWI_ROOT: {os.environ['KIWI_ROOT']}")
...
def main() -> int:
    from kiwi.util.default_root import DEFAULT_ROOT_PATH
