...
from kiwi.daemon.server import singleton, service_launch_lock_path
from kiwi.server.ssl_context import kiwi_ssl_ca_paths, private_ssl_ca_paths
...
class Service:
    def __init__(
        ...
        proctitle_name = f"kiwi_{service_name}"
        ...
        kiwi_ca_crt, kiwi_ca_key = kiwi_ssl_ca_paths(root_path, self.config)
        inbound_rlp = self.config.get("inbound_rate_limit_percent")
        outbound_rlp = self.config.get("outbound_rate_limit_percent")
        assert inbound_rlp and outbound_rlp
        self._server = KiwiServer(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for KiwiServer to be closed")
